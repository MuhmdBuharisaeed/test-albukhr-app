...
async function safeFundRewardFromLiquidity(
  ...
  if(amount <= 0){
    return {
      error:"Invalid reward funding amount"
    };
  }
...
async function getEcosystemLiquidityTotals(){
  ...
  return {
    ...
    total_liquidity:
      liquidityRound(totalLiquidity),

    total_reserve:
      liquidityRound(totalReserve),

    total_usable_liquidity:
      liquidityRound(totalUsable),

    core_liquidity:
      liquidityRound(coreLiquidity),

    internal_liquidity:
      liquidityRound(internalLiquidity),
