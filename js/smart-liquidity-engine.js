...
/*
  ...
  3) js/project-treasury.js

  NETWORK RULE:
  This engine NEVER accepts a caller-selected network.
  It always resolves the current ALBUKHR network through
  the environment/network core and delegates treasury
  reads/writes to the network-aware treasury engine.

  MAINNET => mainnet treasury only
  TESTNET => testnet treasury only
*/
...
function getLiquidityNetwork(){
  ...
  if(
    ...
    throw new Error(
      "ALBUKHR Network Core is required before smart-liquidity-engine.js."
    );
...
function assertLiquidityDependencies(){
  ...
  if(
    typeof projectInternalWithdraw !==
    "function"
  ){
...
async function getLiquidityProjectRules(
  projectCode
){
...
async function getProjectTreasuryStatus(
  projectCode
){
  ...
  /*
    Defensive isolation check.

    project-treasury.js already filters by the
    current network. This second check prevents
    an incorrectly shaped treasury object from
    being consumed by the liquidity layer.
  */
...
async function safeAddProjectLiquidity(
  projectCode,
  amount,
  meta={}
){
...
async function safeProjectInternalWithdraw(
  projectCode,
  amount,
  meta={}
){
...
async function safeFundRewardFromLiquidity(
  projectCode,
  amount,
  meta={}
){
...
async function distributeProjectRewards(
  projectCode,
  stakeRows=[]
){
...
async function getSmartLiquidityGroupedByType(){
  ...
  return {
    ...
    core:
      rows.filter(
        r => r.project_type === "core"
      ),

    internal:
      rows.filter(
        r => r.project_type === "internal"
      ),
...
window.getLiquidityProjectRules =
  getLiquidityProjectRules;

window.getProjectTreasuryStatus =
  getProjectTreasuryStatus;

window.checkProjectLiquidity =
  checkProjectLiquidity;

window.hasMinimumLiquidity =
  hasMinimumLiquidity;

window.canUseLiquidity =
  canUseLiquidity;

window.safeAddProjectLiquidity =
  safeAddProjectLiquidity;

window.safeProjectInternalWithdraw =
  safeProjectInternalWithdraw;

window.safeFundRewardFromLiquidity =
  safeFundRewardFromLiquidity;

window.calculateRewardAmount =
  calculateRewardAmount;

window.distributeSingleStakeReward =
  distributeSingleStakeReward;

window.distributeProjectRewards =
  distributeProjectRewards;

window.getSmartLiquiditySummary =
  getSmartLiquiditySummary;

window.getAllSmartLiquiditySummaries =
  getAllSmartLiquiditySummaries;

window.getSmartLiquidityGroupedByType =
