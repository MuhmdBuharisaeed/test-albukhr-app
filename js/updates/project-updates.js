...
function getSupabaseClient(){
  ...
  if(typeof window.getAlbukhrSupabaseClient === "function"){
    try{
      ...
      if(isValidSupabaseClient(client)) return client;
    }catch(err){
  ...
  for(const candidate of fallbacks){
    if(isValidSupabaseClient(candidate)) return candidate;
  }
...
async function uploadProjectUpdateImage(file, {
  ...
  if(uploadError){
    throw new Error(
      uploadError.message || "Failed to upload update image"
    );
  }
...
async function createProjectUpdate(payload = {}){
  ...
  if(error){
    throw new Error(
      error.message || "Failed to create project update"
    );
  }

  dispatchFeedEvent({ type:"create", update:data });
  return data;
}
...
async function uploadProjectUpdateToSupabase({
  ...
  if(!pickedFile) throw new Error("Project update image is required");
...
async function fetchProjectUpdates({
  ...
  if(projectCode) query = query.eq("project_code", safeString(projectCode).trim());
  if(projectType){
    query = query.eq(
      "project_type",
  ...
  if(error){
    throw new Error(
      error.message || "Failed to fetch project updates"
    );
  }
...
async function fetchProjectUpdateComments(updateId, {
  ...
  if(error){
    throw new Error(
      error.message || "Failed to fetch update comments"
    );
  }
...
async function fetchCommentsMapForUpdates(updateIds = []){
  ...
  if(error){
    throw new Error(
      error.message || "Failed to fetch comments map"
    );
  }
...
async function fetchReactionsMapForUpdates(updateIds = [], viewerEmail = ""){
  ...
  if(error){
    throw new Error(
      error.message || "Failed to fetch update reactions"
    );
  }
  ...
  safeArray(data).forEach(row => {
    ...
    if(!counts[updateId]) counts[updateId] = { like:0, dislike:0 };

    if(vote === "like") counts[updateId].like += 1;
    else if(vote === "dislike") counts[updateId].dislike += 1;
...
function normalizeCommentRow(comment = {}){
  return {
    ...
    commenter_name:
      safeString(comment.commenter_name).trim() || "User",
    commenter_role:
      safeString(comment.commenter_role).trim() || "user",
...
function normalizeUpdateRow(update = {}, {
  ...
  const normalizedComments =
    safeArray(comments).map(normalizeCommentRow);
...
async function fetchProjectUpdatesFeed({
  ...
  return updates.map(update => {
    ...
    const reactionCount =
      reactions.counts[id] || { like:0, dislike:0 };
...
async function fetchTransparencyFeed({
  ...
  if(!email) email = getTransparencyViewerMeta().email;
...
async function getUserReactionForUpdate(updateId, reactorEmail){
  ...
  if(!updateId || !reactorEmail) return null;
  ...
  if(error){
    throw new Error(
      error.message || "Failed to fetch user reaction"
    );
  }
...
async function toggleProjectUpdateReaction({
  ...
  if(!updateId) throw new Error("updateId is required");
  if(!reactorEmail) throw new Error("reactorEmail is required");
  if(!isValidVoteType(voteType)){
    throw new Error("voteType must be like or dislike");
  }

  const existing =
    await getUserReactionForUpdate(updateId, reactorEmail);

  if(
    existing &&
    safeString(existing.vote_type).trim().toLowerCase() === voteType
  ){
    ...
    if(deleteError){
      throw new Error(
        deleteError.message || "Failed to remove reaction"
      );
    }
...
async function toggleTransparencyReaction(
  updateId,
  voteType,
  viewerMeta = null
){
...
async function getProjectUpdateStats(updateId){
  ...
  if(reactionsRes.error){
    throw new Error(
      ...
      "Failed to fetch reactions stats"
    );
  }
  ...
  if(commentsRes.error){
    throw new Error(
      ...
      "Failed to fetch comments stats"
    );
  }
...
async function deleteProjectUpdate(updateId){
  ...
  if(error){
    throw new Error(
      error.message || "Failed to delete project update"
    );
  }

  dispatchFeedEvent({ type:"delete", updateId });
  return true;
}
...
async function migrateLegacyLocalProjectFeedToSupabase({
  ...
  for(const item of legacyFeed){
    try{
      const projectCode =
        safeString(item.project_code || item.project).trim();
