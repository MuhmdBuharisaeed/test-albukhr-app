...
ENGINE CORE + HELPERS + SESSION + ACCESS + RPC + SUBMISSION
...
(function () {
  ...
  /* =========================================================
     INTERNAL SESSION VALIDATION / ENTRY GATE
  ========================================================= */
  ...
  function validateInternalProjectPayload(payload = {}) {
    ...
    if (payload.initialLiquidity !== undefined) {
      const liquidity =
        Number(payload.initialLiquidity);

      if (!Number.isFinite(liquidity)) {
        errors.push(
          "Initial liquidity must be a valid number."
        );
      }

      if (liquidity < 0) {
        errors.push(
          "Initial liquidity cannot be negative."
        );
      }
    }
  ...
  InternalRegistryEngine.BUILD =
    "SUPABASE-PRODUCTION-CONSOLIDATED";
