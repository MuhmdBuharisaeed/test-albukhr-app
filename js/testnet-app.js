...
async function boot(){
  try{
    ...
    if(!window.AlbukhrTestnetAuth){
      throw new Error(
        "Testnet Auth module is unavailable."
      );
    }

    /*
     * Only the secure Mainnet -> Testnet access-code flow
     * or an existing short-lived Testnet session is accepted.
     *
     * We deliberately removed the previous referrer-based
     * bypass. A referrer is not an authentication mechanism.
     */
    const session=
    ...
    if(!window.ALBukhrEnvironment){
      throw new Error(
        "Environment Core is unavailable."
      );
    }
    ...
    stage="REGISTRY_RPC";
    setStatus(
      "REGISTRY RPC • Loading approved Testnet projects…"
    );
