/* =========================================================
   ...
   - Network-aware Mainnet/Testnet isolation
   - Logo/image first; no emoji project identity
   - Requires js/supabase-core.js + network core
========================================================= */
(function(window){
"use strict";
...
function mime(v){
  v=str(v).trim().toLowerCase();
  return v==="image/png" || v==="image/jpeg" || v==="image/webp" ? v : "";
}
...
function logoValid(r){
  ...
  const media=!!str(r.logo_url).trim() || !!str(r.image_url).trim() || !!str(r.logo_path).trim();
  return !!m && Number.isFinite(s)&&s>0&&s<=1048576 &&
         Number.isFinite(w)&&Number.isFinite(h)&&w>=400&&h>=400&&media;
}

function normalize(r={}){
  const code=str(r.project_code||r.code||r.slug).trim();
  const name=str(r.project_name||r.title||r.name||code||"Unnamed Project").trim();
  const imageUrl=str(r.logo_url||r.image_url||r.logo||r.image).trim();

  return {
    id:r.id??null,
    network:str(r.network||getNetwork()).trim().toLowerCase(),
    project_code:code,
    code,
    project_name:name,
    name,
    ...
    // Legacy field retained only for compatibility; never used as identity.
    icon:"",

    description:str(r.description||r.desc,"ALBUKHR Project"),
    info:str(r.info||r.about||r.details,"Project information not available."),
    durations:durations(r.durations),

    reserve_percent:num(r.reserve_percent,.30),
    min_liquidity:num(r.min_liquidity,100),
    min_stake:num(r.min_stake,r.min_liquidity ?? 100),
    ...
    logo_required:true,
    logo_present:!!(imageUrl||str(r.logo_path).trim()),
...
function sortRows(rows){
  return [...rows].sort((a,b)=>
    str(a.project_name).localeCompare(str(b.project_name),undefined,{sensitivity:"base"})
  );
}
...
async function fetchProjectsFromSupabase(){
  ...
  if(!network) return {success:false,error:"ALBUKHR network is not available"};
  ...
  try{
    ...
    return {success:true,data:sortRows(rows),network};
  }catch(e){
    return {success:false,error:e?.message||"Projects fetch failed"};
  }
}
...
async function loadProjects(force=false){
  ...
  if(loaded && lastNetwork===network && !force) return cache;
  if(loading && lastNetwork===network && !force) return cache;
  ...
  if(!r.success) console.warn("projects-engine-v4:",r.error);
  return cache;
}
...
async function getProjectsByType(t,o={}){
  const rows=await getAllProjects(o);
  t=type(t);
  return rows.filter(p=>p.project_type===t);
}
...
async function groupProjectsByType(o={}){
  const r=await getAllProjects(o);
  return {
    core:r.filter(p=>p.project_type==="core"),
    internal:r.filter(p=>p.project_type==="internal"),
    external:r.filter(p=>p.project_type==="external")
  };
}

async function getProjectByCode(code,o={}){
  if(!code) return null;
  const c=str(code).trim().toLowerCase();
  return (await getAllProjects(o))
    .find(p=>str(p.project_code).trim().toLowerCase()===c) || null;
}

async function getProjectMeta(c){ return getProjectByCode(c); }
async function getProject(c){ return getProjectByCode(c); }

async function getProjectLogo(c){
  const p=await getProjectByCode(c);
  return p?.logo_url||null;
}
async function getProjectLogoUrl(c){ return getProjectLogo(c); }

async function getProjectLogoPath(c){
  const p=await getProjectByCode(c);
  return p?.logo_path||null;
}

async function isProjectLogoReady(c){
  const p=await getProjectByCode(c);
  return !!p&&p.logo_valid===true;
}
async function hasProjectLogo(c){
  const p=await getProjectByCode(c);
  return !!p&&p.logo_present===true;
}
async function getProjectTitle(c){
  const p=await getProjectByCode(c);
  return p?.project_name||c||"Unknown Project";
}
async function getProjectName(c){ return getProjectTitle(c); }

// Deliberately returns empty string: emoji is not project identity.
async function getProjectIcon(){ return ""; }

async function getProjectDescription(c){
  const p=await getProjectByCode(c);
...
async function getProjectDurations(c){
  const p=await getProjectByCode(c);
  return p?.durations||[30,60,90];
}
async function getProjectType(c){
  const p=await getProjectByCode(c);
  return p?.project_type||null;
}
async function getProjectStatus(c){
  const p=await getProjectByCode(c);
  return p?.status||null;
}
async function projectExists(c){ return !!(await getProjectByCode(c)); }
async function isProjectActive(c){
  const p=await getProjectByCode(c);
  return !!p&&p.status==="active";
}
async function isProjectVisible(c){
  const p=await getProjectByCode(c);
  return !!p&&p.is_visible!==false;
}
async function isCoreProject(c){
  const p=await getProjectByCode(c);
  return !!p&&p.project_type==="core";
}
async function isInternalProject(c){
  const p=await getProjectByCode(c);
  return !!p&&p.project_type==="internal";
}
async function isExternalProject(c){
  const p=await getProjectByCode(c);
  return !!p&&p.project_type==="external";
}
async function isProjectTreasuryEnabled(c){
  const p=await getProjectByCode(c);
  return !!p&&p.treasury_enabled!==false;
}
async function isProjectStakingEnabled(c){
  const p=await getProjectByCode(c);
  return !!p&&p.staking_enabled!==false;
}
async function isProjectContributionsEnabled(c){
  const p=await getProjectByCode(c);
  return !!p&&p.contributions_enabled!==false;
}
async function getProjectRules(c){
  const p=await getProjectByCode(c);
  return {
    reserve_percent:num(p?.reserve_percent,.30),
    min_liquidity:num(p?.min_liquidity,100),
    min_stake:num(p?.min_stake,p?.min_liquidity ?? 100),
    reward_rate:num(p?.reward_rate,.02)
  };
}

async function getProjectsEngineSummary(){
  const a=await getAllProjects();
  const g=await groupProjectsByType();
  return {
    total:a.length,
    network:getNetwork(),
    core:g.core.length,
    internal:g.internal.length,
    external:g.external.length,
    logo_ready:a.filter(p=>p.logo_valid).length,
    logo_missing:a.filter(p=>!p.logo_valid).length,
    loaded,
    loading,
...
window.addEventListener("DOMContentLoaded",()=>{
  loadProjects().catch(e=>console.warn("Projects preload warning:",e));
});

console.log("ALBUKHR Projects Engine v4 — Supabase + Network + Logo First");
